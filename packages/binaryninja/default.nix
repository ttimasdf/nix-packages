    ...
    stdenv.mkDerivation (finalAttrs: {
      ...
      desktopItems = [
        (makeDesktopItem {
          ...
          icon = "binaryninja";
          desktopName =
            "Binary Ninja ${lib.toSentenceCase edition}" + (if isDev then " (Dev Channel)" else "");
          mimeTypes = [
      ...
      meta = with lib; {
        changelog = "https://binary.ninja/changelog/#${replaceStrings [ "." ] [ "-" ] finalAttrs.version}";
  ...
  packages = lib.flatten (
    lib.mapAttrsToList (
      ...
      let
        ...
        stablePkg =
          if latestStableVersion != null then
            {
              name = formatPname {
                inherit edition;
                isDev = false;
                ...
                (mkBinaryNinjaDerivation {
                  ...
                  (old: {
                    passthru.allVersions = lib.mapAttrs' (
                      v: h:
                      lib.nameValuePair v (mkBinaryNinjaDerivation {
                        version = v;
                        hash = h;
                        inherit edition;
                      })
