...
The package overlay is `overlays.default` and should normally be enabled first. It provides this repository's packages, `makeDesktopItemExtended`, `makeSanitizedLauncherHook`, and the compatibility attributes needed by Wuying Cloud Desktop (`qt5w` and `libtiff-abi5`).

Other overlays are opt-in because they change or replace existing Nixpkgs packages. Apply them alongside the default overlay in a NixOS configuration:
...
| `libtiff5` | Exposes `pkgs.libtiff-abi5`, imported from a pinned Nixpkgs revision for binaries requiring the old ABI. Included by `overlays.packages`. |
| `nvtop` | Adds `pkgs.nvtopPackages.nvidia-intel`, built with both Intel and NVIDIA support. |
| `qt5webengine` | Exposes `pkgs.qt5w`, a pinned Qt 5 package set containing the legacy Qt WebEngine required by Wuying. Included by `overlays.packages`. |
...
Use one of these overrides explicitly; applying every opt-in overlay can introduce compatibility pins, duplicate package attributes, or older dependencies that unrelated packages do not need.
